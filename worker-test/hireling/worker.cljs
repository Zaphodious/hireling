...
(hc/start-service-worker! {; As service workers are updated in the browser when the file itself is byte-different
                           ...
                           ; one already present, it will abort installation (note 1). If a hireling-defined
                           ; service worker finds that it has the same version number as the one previously
                           ; installed, it will ensure that its :cached-paths are up-to-date and then
                           ; immediately take over using the same cache. If a hireling-defined service worker
                           ; finds that it has a greater version number then the one already installed, it
                           ...
                           :cached-paths      {; Paths under :cache-never are never cached. Offline
                                               ...
                                               ; situations. Provides no performance benefit. Use sparingly.
                                               :cache-last    [""]
                           ...
                           ; after max-requests has been exceeded.
                           :send-later        {(constantly false) [0, 0]}

                           ; The :fallback option takes a urlstring for a fallback page that will be shown when the
                           ; user is reasonably thought to be offline and all other caching has failed. Last resort,
                           ; explicitly to avoid seeing the browser's "sorry, you're offline" page. Stored separately
                           ; from other data.
                           :fallback          ""

                           ; The :cache-intercept entry allows the developer to fully take control of
                           ; response behavior without overriding everything. It takes a map of
                           ...
                           ; async, so regular returns are often impossible. Included as a safety valve in case
